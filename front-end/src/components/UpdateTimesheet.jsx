...
import { Box, Divider, Flex, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Select, useDisclosure } from "@chakra-ui/react";
...
function UpdateTimesheet({ timesheet }) { 
    ...
    const { data, handleChange, handleEdit, setData } = useTimesheet(setShowNotification);

    useEffect(() => {
        if (isOpen && data) {
            setSelectedOption(data.nama_proyek); 
            setNewProyekName(data.nama_proyek);
        }
    }, [isOpen, data]);
        ...
        <>
            ...
            <Modal isOpen={isOpen} onClose={onClose}>
                ...
                <ModalContent maxWidth='700px'>
                    <ModalHeader>Update kegiatan</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
                        <Divider />
                        <form onSubmit={handleSubmitEdit}>
                            <Flex gap={4} mt={4} mb={4}>
                                <Box>
                                    ...
                                    <Input placeholder='13 Okt 2023' size='md' type='date' name="tanggal_mulai" onChange={handleChange} value={data ? data.tanggal_mulai : ''} />
                                </Box>
                                <Box>
                                    <Text mb={3}>Tanggal Berakhir</Text>
                                    <Input placeholder='13 Okt 2023' size='md' type='date' name="tanggal_berakhir" onChange={handleChange} value={data ? data.tanggal_berakhir : ''} />
                                </Box>
                                <Box>
                                    <Text mb={3}>Jam Mulai</Text>
                                    <Input placeholder='08:00' size='md' type='time' name="jam_mulai" onChange={handleChange} value={data ? data.jam_mulai : ''} />
                                </Box>
                                <Box>
                                    <Text mb={3}>Jam Berakhir</Text>
                                    <Input placeholder='17:00' size='md' type='time' name="jam_berakhir" onChange={handleChange} value={data ? data.jam_berakhir : ''} />
                                </Box>
                            </Flex>
                            <Box>
                                <Text mb={4}>Judul Kegiatan</Text>
                                <Input size='md' mb={4} name="judul_kegiatan" onChange={handleChange} value={data ? data.judul_kegiatan : ''} />
                            </Box>
                            <Box>
